...
def process_output_llms(output:list, k=0)->list:
    # Regular expression to extract the occupational and sector labels
    # patterns = [r".*Occupational label: (.*?)\.\nSector label: (.*?)\.$",
    #             r".*Occupational label: (.*?)\n.*Sector label: (.*?)$",
    #             ]
    pattern = r".*Occupational label: (.*?)\n.*Sector label: (.*?)\].*$"
    ...
    # print("match: ", match)
    if match:
            occupational_label = match.group(1).strip(".")
            sector_label = match.group(2).strip(".")
    else:
        occupational_label = "NaN"
        sector_label = "NaN"
        k+=1
        raise ValueError("Warning: Could not find the occupation or sector label")
    

    print("Occupational Label:", occupational_label)
    print("Sector Label:", sector_label)
    print("--------------------------------")

    return occupational_label, sector_label, k
...
def parse_arguments():
    ...
    parser.add_argument("--use_quantization", type=str, help="use quantization", default='False')
    #parse arguments
    return parser.parse_args()

# CUDA_VISIBLE_DEVICES==1 python main_inference.py --model_name meta-llama/Meta-Llama-3-8B-Instruct --use_quantization True
# CUDA_VISIBLE_DEVICES=1 python main_inference.py --model_name meta-llama/Llama-3.2-1B-Instruct --use_quantization True
# CUDA_VISIBLE_DEVICES=1 python main_inference.py --model_name meta-llama/Llama-3.1-8B-Instruct --use_quantization True

if __name__ == "__main__":
    torch.cuda.empty_cache()
    args = parse_arguments()
    model_name = args.model_name
    print(args.use_quantization)
...
"""
    ...
    for i in range(len(my_prompts['occupations'])):
